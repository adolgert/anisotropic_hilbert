...
static inline u32 mask_bits(u32 bits) {
  if (bits >= 32u) return 0xFFFFFFFFu;
  return (bits == 0u) ? 0u : ((1u << bits) - 1u);
}

static inline u32 rotl_bits(u32 x, u32 r, u32 bits) {
  if (bits == 0u) return x;
  x &= mask_bits(bits);
  r %= bits;
  if (r == 0u) return x;
...
// Matches hilbert_affine.c: BRGC rotated so the whole Gray path exits on axis 0.
static std::vector<u32> gray_axis0_seq(int k) {
  const u32 n = 1u << k;
  std::vector<u32> out(n);
  for (u32 w = 0; w < n; w++) {
    out[w] = rotl_bits(brgc(w), 1u, (u32)k);
  }
  return out;
}
...
static void write_header(const std::string& path, const std::vector<Tables>& all) {
  ...
  out << "// Tables for alternative Hilbert child geometry (2D..7D).\n\n";
  out << "#pragma once\n\n";
...
int main() {
  ...
  const std::string header_path = "/mnt/data/hilbert_generated_tables.h";
