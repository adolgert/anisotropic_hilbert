...
static inline uint32_t trailing_zeros(uint32_t x)
{
  /* x must be nonzero */
  uint32_t c = 0;
  while ((x & 1u) == 0u)
...
static inline uint32_t child_entry_hamilton(uint32_t w, uint32_t k)
{
  if (w == 0u)
    return 0u;
...
static inline uint32_t child_dir_hamilton(uint32_t w, uint32_t k)
{
  if (w == 0u)
    return 1u;
  if ((w & 1u) != 0u)
...
/*
 * Alternative BRGC/Hilbert variant driven by the mismatch walk s_w.
 *
 * Keeps the same rotated BRGC on k bits (exit at axis 0), but uses a different
 * closed-form mismatch sequence s_w.  This changes the child entry corners and
 * child exit directions, yielding a different self-similar Hilbert-like curve.
 *
 * Conventions here match this file:
 *   - High bits use gray_code_axis0(w,k) = rotL(BRGC(w), 1).
 *   - The mismatch variable is s_w = h_w XOR ell_in(w).
 *
 * Define hub = (k==1 ? 0 : 1).  Then:
 *   s_0 = 0
 *   s_1 = e_hub
 *   s_{2^k-1} = e_hub
 *   s_w = e_hub XOR e_{r(w)}                        for even w>0
 *   s_w = e_hub XOR e_{r(w-1)} XOR e_{r(w+1)}        for odd w not in {1,2^k-1}
 * where r(t) = (ctz(t)+1) mod k (note: t is even in all r() uses above).
 *
 * Entry corners are:
 *   ell_in(w) = h(w) XOR s_w, with h(w)=gray_code_axis0(w,k).
 *
 * Exit direction is:
