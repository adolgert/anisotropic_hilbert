...
\newcommand{\vtimes}{\mathbin{\vtimes}}
...
Let $Q_k=\{0,1\}^k\cong \mathbb{F}_2^k$.

A directed Gray code in dimension $k$ is a Hamiltonian path
$$
    \gc_k : \{0,\ldots,2^k-1\} \rightarrow Q_k
$$
such that consecutive vertices differ in exactly one coordinate.
$$
    \gc_k(i+1) = \gc_k(i)\XOR \epsilon_{\sigma_k(i)}\quad\text{for some}\quad \sigma_k(i)\in\{0,\ldots,k-1\}.
$$
Equivalently, we can define the step vector
$$
    g_k(i)=\gc_k(i)\XOR\gc_k(i+1)\in\{\epsilon_0,\ldots,\epsilon_{k-1}\}.
$$
...
A parent hypercube has $k$ dimensions and is dyadic, so it's of extent 2 in
...
that depends on the particular child $i$ (call it $S_i$), then the adjacency criterion is
that $\omega_{i} +\alpha_{i+1} = \epsilon_g$. $\epsilon_g$ is along the direction of the Hilbert
curve, so $\gc(i) + \gc(i+1) = \epsilon_g$.

$$
\begin{aligned}
    \alpha_i & = S_i(\mathbf{0})=e_i \\
    \omega_i & = S_i(\mathbf{0} + \epsilon_0)= \rho^{d_i}\epsilon_0 + e_{i}=\epsilon_{d_i}+ e_{i} \\
    \alpha_{i+1} & = e_{i+1}
\end{aligned}
$$

Work through that. All values are in dimension $k$.
$$
\begin{aligned}
    \omega_i + \alpha_{i+1} & = g_k(i) \\
    \epsilon_{d_i}+ e_{i}+e_{i+1} & = g_k(i)
\end{aligned}
$$

A $k$-dimensional Hilbert curve is defined by a set of child transforms
$\{S_i^{(k)}\}_{i=0}^{2^k-1}$ such that, after placing child $i$ with $S_i^{(k)}$,
the exit of child $i$ meets the entry of child $i+1$ across the shared face.
...
$$
\begin{aligned}
    \omega_i + \alpha_{i+1} & = g_{(k+1)}(i) \\
    \epsilon_{d_i+ \dk}+ \rho^{\dk} e_{i}+\rho^{\dk}e_{i+1} & = \rho^{\dk}g_{(k+1)}(i)
\end{aligned}
$$
