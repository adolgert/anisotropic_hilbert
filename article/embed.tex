...
\begin{aligned}
    ...
    \omega_i & = S_i(\mathbf{0} + \epsilon_0)= \rho^{d_i}\mathbf{0} + \rho^{d_i}\epsilon_0 + e_{i}=\epsilon_{d_i}+ e_{i} \\
    \alpha_{i+1} & = e_{i+1}
\end{aligned}
$$
...
$$
\begin{aligned}
    \omega_i + \alpha_{i+1} & = \epsilon_g \\
    \epsilon_{d_i}+ e_{i}+e_{i+1} & = \epsilon_g
\end{aligned}
$$
...
If it is true that $\epsilon_{d_i}+ e_{i}+e_{i+1} = \epsilon_g$ then when we
modify them as above,
$$
\begin{aligned}
    \alpha_i & = S'_i(\mathbf{0})=\rho^{\Delta}e_i \\
    \omega_i & = S'_i(\mathbf{0} + \epsilon_0)=
        \rho^{d_i + \Delta}\mathbf{0} + \rho^{d_i + \Delta}\epsilon_0 + \rho^{\Delta}e_{i}
        =\epsilon_{d_i+ \Delta}+ \rho^{\Delta} e_{i} \\
    \alpha_{i+1} & = \rho^{\Delta}e_{i+1}
\end{aligned}
$$
In the $k+\Delta$ dimensions this becomes:
$$
\begin{aligned}
    \omega_i + \alpha_{i+1} & = \epsilon'_g \\
    \epsilon_{d_i+ \Delta}+ \rho^{\Delta} e_{i}+\rho^{\Delta}e_{i+1} & = \epsilon'_g
\end{aligned}
$$
What happens to $\epsilon_g$ in the new dimensions? We need it to be true
that the embedded Gray code is $\rho^{\Delta}\epsilon_g$ compared to the $k$-dimensional one.
