...
\begin{algorithmic}[1]
\Require Dimension $n$; parent Gray order $g[0..2^n!-!1]\subset GF(2)^n$ with $g[0]=0$
...
\begin{algorithm}
\caption{Two-level $4^n$ child-entry/exit tables via mismatch-state DP in $\mathrm{GF}(2)^n$}
\label{alg:gf2-dp}
\begin{algorithmic}[1]
\Require Dimension $n\ge 2$; parent Gray sequence $g_0,\dots,g_{2^n-1}\in\mathrm{GF}(2)^n$ with $g_0=0$
\Ensure Child entry corners $\ell^{\mathrm{in}}_w\in\mathrm{GF}(2)^n$ and exit directions $\mathrm{dir}_w\in\{0,\dots,n-1\}$ for $w=0,\dots,2^n-1$

\State $N\gets 2^n$, \quad $m\gets N-1$
\For{$w\gets 0$ \textbf{to} $m-1$}
  \State Let $d_w$ be the unique index such that $g_{w+1}=g_w\oplus e_{d_w}$
\EndFor

\State Define mismatch state $s_w := g_w \oplus \ell^{\mathrm{in}}_w \in \mathrm{GF}(2)^n$
\State Initialize reachable set $R_0\gets\{0\}$ and predecessor map $\pi(\cdot)\gets\bot$

\For{$w\gets 0$ \textbf{to} $m-1$}
  \State $R_{w+1}\gets\emptyset$
  \ForAll{$s\in R_w$}
    \For{$a\gets 0$ \textbf{to} $n-1$}
      \State $s' \gets s\oplus e_a$ \Comment{$s_{w+1}=s_w\oplus e_{\mathrm{dir}_w}$}
      \If{$(s')_{d_w}=1$} \Comment{face constraint for crossing cube $w\to w+1$}
        \If{$s'\notin R_{w+1}$}
          \State $R_{w+1}\gets R_{w+1}\cup\{s'\}$
          \State $\pi(w+1,s')\gets (s,a)$
        \EndIf
      \EndIf
    \EndFor
  \EndFor
...
\State Choose any $s_{m}\in R_m$ with Hamming weight $1$
\State Backtrack with $\pi$ to recover $s_0,\dots,s_m$ and $a_0,\dots,a_{m-1}$
\For{$w\gets 0$ \textbf{to} $m$}
  \State $\ell^{\mathrm{in}}_w \gets g_w \oplus s_w$
  \State $\mathrm{dir}_w \gets a_w$ \Comment{for $w<m$}
\EndFor
\State $\mathrm{dir}_m \gets \text{index of the 1-bit of } s_m$ \Comment{last cube ends at local corner $g_m$}

\Return $\{\ell^{\mathrm{in}}_w,\mathrm{dir}_w\}_{w=0}^{m}$
\end{algorithmic}
\end{algorithm}
