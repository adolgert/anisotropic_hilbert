...
[
(\ell_k\oplus e_{a_k})*{d_k} = 1 - (h_k)*{d_k}.
]
XOR both sides with ((h_k)*{d_k}) and use (s_k=h_k\oplus \ell_k), plus (1), to rewrite it as the very clean condition:
[
(s*{k+1})_{d_k} = 1.
\tag{2}
]
...
> and ((s_{k+1})*{d_k}=1) for every (k=0,\dots,2^n-2).
> Then set (\ell_k := h_k\oplus s_k) and (a_k := ) the bit flipped between (s_k) and (s*{k+1}).
